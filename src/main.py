...
def run_publisher(account_name: str) -> tuple:
    """
    Pipeline complet TITAN V3.
    
    Returns:
        (status: str, should_fail: bool)
        - status: "success" | "rate_limited" | "no_video" | "error"
        - should_fail: False pour rate_limited/no_video (exit 0), True pour erreur vraie (exit 1)
    """
    logger.info("=" * 80)
    ...
    if not allowed:
        ...
        # ⭐ IMPORTANT: rate_limited = pas d'erreur GitHub, exit 0
    ...
    if not force_post:
        if not config.schedule.is_publishing_time(now.hour, now.minute):
            ...
            # ⭐ Hors plage = pas d'erreur GitHub, exit 0
            return "rate_limited", False
    ...
    if not video:
        ...
        # ⭐ Pas de video = pas d'erreur GitHub, exit 0
        return "no_video", False
    ...
    try:
        ...
        if success:
            ...
            # ⭐ Upload failed = erreur vraie, exit 1
            return "error", True
...
def main() -> int:
    """
    Exit codes:
    - 0: Succès OU rate_limited OU no_video (workflow success)
    - 1: Erreur vraie (crash, upload failed, etc) - GitHub notifié
    """
    account_name = os.environ.get("ACCOUNT_NAME", "youtube_compte1")
    try:
        status, should_fail = run_publisher(account_name)
        
        logger.info("=" * 80)
        logger.info(f"Status: {status.upper()} | Exit: {1 if should_fail else 0}")
        logger.info("=" * 80)
        
        # ⭐ LOGIQUE SMART:
        # - success, rate_limited, no_video → exit 0 (pas de spam GitHub)
        # - error → exit 1 (GitHub notifié légitimement)
